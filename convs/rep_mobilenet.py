...
class conv_block(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias, mode=''):
        super(conv_block, self).__init__()
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias)
        self.mode = mode
        if mode == 'parallel_adapters':
            self.adapter = conv1x1(in_channels, out_channels, stride, groups)
    ...
    def __init__(
        ...
        layers.extend(
            [
                ...
                # pw-linear
                # conv_block(hidden_dim, oup, 1, 1, 0, dilation=1, groups=1, bias=False, mode=mode),
...
class Rep_MobileNetV2(nn.Module):
    def __init__(
            self,
